...
class ConfirmacaoUsuarioPagadorAutorizacaoPixAutomaticoJobConfigTest {
    ...
    @InjectMocks
    private ConfirmacaoUsuarioPagadorAutorizacaoPixAutomaticoJobConfig jobConfig;
    ...
    void dadoCronExpression_quandoConfigureTask_deveDefinirIntervaloCorretamente() {
        ...
        when(propertiesLoader.getJobConfirmacaoUsuarioPagadorAutorizacaoPixAutomatico()).thenReturn(jobShedLock);
        when(jobShedLock.getCronExpression()).thenReturn(cronExpression);

        jobConfig.configureTasks(taskRegistrar);
